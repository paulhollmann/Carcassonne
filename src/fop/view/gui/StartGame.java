...
public class StartGame extends View implements GameConstants {
	...
	public void onResize() {
		...
		lblPlayerCount.setLocation(
				(this.getWidth() - lblPlayerCount.getWidth() - 25) / 2 - lblPlayerCount.getWidth() / 2, offsetY + 2);
		playerCount.setLocation(
				(this.getWidth() - playerCount.getWidth() - 25) / 2 + lblPlayerCount.getWidth() / 2 + 10, offsetY);
		offsetY += 50;
	...
	protected void onInit() {
		...
		for (int i = 0; i < GameConstants.MAX_PLAYERS; i++) {
			playerConfig[i] = new JComponent[] { createLabel(String.format("%d.", i + 1), 16),
					new ColorChooserComboBox().getComboBox(), new JTextField(String.format("Player %d", i + 1)) };
			...
			try {
				...
				for (int i = 0; i < playerCount; i++) {
					...
					if (name.isEmpty()) {
						...
						showErrorMessage(String.format("Please indicate a valid name for a player.", i + 1),
								"Invalid Data");
					...
					if (!Players.isColorAvailable(currentPlayer)) {
						...
						Players.resetPlayers();
						showErrorMessage(String.format("There are two players with the same color"), "Same Color");
						return;
					} else {
